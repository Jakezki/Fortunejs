...
function main() {
    const response = prompt("Wanna hear a fortune? (yes/no): ");
    if (response && (response.toLowerCase() === 'yes' || response.toLowerCase() === 'y')) {
        const randomIndex = Math.floor(Math.random() * quotes.length);
        alert(quotes[randomIndex]);
    } else if (response && (response.toLowerCase() === 'no' || response.toLowerCase() === 'n')) {
        alert("Too bad, you filthy monkey!");
    } else {
